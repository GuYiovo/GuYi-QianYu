...
if (file_exists($lock_file)) {
    ...
    if ($lock_data && $lock_data['expire'] > time()) {
        $remaining = ceil(($lock_data['expire'] - time()) / 60);
        die("<div style='font-family:sans-serif;text-align:center;padding:50px;color:#ef4444;'>
            <h2>🚫 访问受限</h2>
            <p>由于密码尝试失败次数过多，您的IP已被暂时锁定。</p>
...
$conf_favicon = $sysConf['favicon'] ?? 'https://q2.qlogo.cn/headimg_dl?dst_uin=156440000&spec=640';
$conf_avatar = $sysConf['admin_avatar'] ?? 'https://q2.qlogo.cn/headimg_dl?dst_uin=156440000&spec=640';

if(isset($sysConf['favicon']) && strpos($sysConf['favicon'], 'http://') === 0) {
    $conf_favicon = str_replace('http://', 'https://', $sysConf['favicon']);
}
if(isset($sysConf['admin_avatar']) && strpos($sysConf['admin_avatar'], 'http://') === 0) {
    $conf_avatar = str_replace('http://', 'https://', $sysConf['admin_avatar']);
}
